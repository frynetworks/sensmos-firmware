...
void mesh_tx_status_json(String& out) {
    ...
    snprintf(b, sizeof(b),
        ...
        "\"portnum\":%lu,\"node_num\":%lu,\"region\":\"%s\",\"freq\":%.3f,\"power\":%d,"
        "\"sf\":%u,\"bw\":%.1f,\"pending\":%s,\"packets\":%lu,\"air_ms\":%lu}",
        s_enabled ? "true" : "false", s_channel,
        s_key.len == 16 ? "aes128-ctr" : "aes256-ctr", s_chan_hash,
        (unsigned long)s_portnum, (unsigned long)mesh_node_num(),
        rg->name, rg->mesh_freq, (int)rg->mesh_power,
        MESH_SF, MESH_BW, s_pending ? "true" : "false",
...
uint32_t mesh_tx_next(LoraRadio& radio, uint32_t duty_ms, uint32_t duty_budget,
    ...
    const char*       band  = rg->band[1].name;
    ...
    if (duty_ms + est > duty_budget) {
        ...
        if (!warned) {
            warned = true;
            LOGW("mesh", "TX paused — %s duty budget spent (%lu/%lums per h, need %lums)",
                 band, (unsigned long)duty_ms, (unsigned long)duty_budget, (unsigned long)est);
        }
        return 0;
    }
    ...
    int st = radio.begin(freq, MESH_BW, MESH_SF, MESH_CR, MESH_SYNCWORD,
    ...
    LOGI("mesh", "radio -> %.3f MHz SF%u BW%.0f sync 0x%02X preamble %u",
         freq, MESH_SF, MESH_BW, MESH_SYNCWORD, MESH_PREAMBLE);
    ...
    s_sent_total++;
    s_air_total += air;
    LOGI("mesh", "packet %u/%u sent @%.3f SF%u (id 0x%08lx, %uB, %lums air, duty %lu/%lums/h %s)",
         (unsigned)(s_frag + 1), (unsigned)s_nfrag, freq, MESH_SF,
         (unsigned long)s_packet_id, (unsigned)flen,
         (unsigned long)air, (unsigned long)(duty_ms + air), (unsigned long)duty_budget, band);
    s_packet_id++;
    if (++s_frag >= s_nfrag) {
        s_pending = false;
