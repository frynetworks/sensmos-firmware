...
//
// ESP32 port of nrf52840/src/mesh_proto.h. Byte-for-byte the same wire format; the
// only substitutions are the crypto backend (mbedtls instead of rweather/Crypto —
// mbedtls ships with the ESP32 core, so no extra library) and the NodeNum source
// (WiFi station MAC instead of the nRF FICR device address).
// ══════════════════════════════════════════════════════════════
...
// This node's Meshtastic NodeNum, folded from the WiFi station MAC — the same source
// identity.cpp already mixes into the device id, so a node's mesh address and its
// SENSMOS id stay tied to the same silicon and survive reboots.
