#!/usr/bin/env python3
# Per-sub-band duty-cycle gate for the nRF52840 port.
#
...
# Charging both protocols to a single 1% counter (the pre-split firmware) makes them
# starve each other: Meshtastic SF11 airtime eats the SMOS budget, and around 36 s/h
# BOTH protocols go silent even though g4 still has ~90% of its allowance unused.
#
...
#
#   py -3 tools/gate_duty_bands.py [--minutes 10] [--out FILE]
#
# Exit 0 = PASS, 1 = FAIL, 2 = environment problem. RED on the shared-counter
# firmware, GREEN once the counters are split. Counters are read from the node's own
# TX / budget-spent log lines rather than from get_info, so the gate depends only on
# what the radio path prints. Port resolved by USB VID:PID, never a COM number.
...
# transmission and pins just under (35988 observed on the pre-split firmware). The
# reference point is therefore "within one small frame of the cap", not "past it".
