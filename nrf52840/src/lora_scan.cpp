...
static void after_begin() {
    ...
    // nRF port: Wio-SX1262 routes the TX side of the RF switch through DIO2.
    if (g_pin->dio2_rf) s_radio.setDio2AsRfSwitch(true);
...
static void beacon_code(char out[9], uint32_t minute) {
    out[0] = 0;
    // nRF port: same HMAC-SHA256, computed with rweather/Crypto instead of mbedtls.
    char msg[32];
    snprintf(msg, sizeof(msg), "%.8s:%lu", g_device_id, (unsigned long)minute);
    uint8_t mac[32];
    SHA256 h;
    h.resetHMAC(s_link.seed, sizeof(s_link.seed));
    h.update((const uint8_t*)msg, strlen(msg));
    h.finalizeHMAC(s_link.seed, sizeof(s_link.seed), mac, sizeof(mac));
...
// ── Batch uplink (nRF port addition) ─────────────────────────────────────────
// Signed data batches leave the node as chunked LoRa frames on the LINK channel:
//   [0xE1]["SMOSB "][id8][' '][seq hex4][' '][idx]['/'][cnt][' '][base64 of 72B slice]
// One frame per second outside the beacon slot and guard windows, each frame
// debited against the same EU868 duty-cycle counter as the beacon. There is no
// gateway protocol upstream — reception is a deployment concern; the node's job
// is to put authenticated data on the air within its airtime budget.
...
#define LORA_UPLINK_RAW_CHUNK ((LORA_UPLINK_CHUNK / 4) * 3)   // 96 b64 chars = 72 raw bytes
...
bool lora_uplink_enqueue(const char* json, size_t len) {
    ...
    if (s_up_pending) return false;                 // one batch in flight at a time
...
// TX one pending uplink frame. Same duty-cycle discipline as the beacon.
...
static bool try_pinout(const LoraPinout& p) {
    // nRF port: SPI pins are fixed by the XIAO variant (D8/D9/D10 for every row of
    // the table) — the ESP32 remap call doesn't exist here and isn't needed.
    SPI.begin();
...
void lora_scan_init() {
    ...
    // nRF port: single-core FreeRTOS (Adafruit core) — stack depth is in WORDS here,
    // not bytes like ESP32: 2048 words = 8KB.
    xTaskCreate(lora_task, "lora", 2048, nullptr, 1, &s_task);
